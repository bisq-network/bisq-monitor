...
package bisq.monitor.monitor.tasks.bisqnetwork;
 ...
 */
@Slf4j
public class P2PNetworkLoad extends MonitorTask implements MessageListener, SetupListener {
    ...
    public P2PNetworkLoad(Properties properties, Reporter reporter) {
    ...
    protected void execute() {
        ...
        if (null == networkNode) {
            ...
            int port = Integer.parseInt(configuration.getProperty(TOR_PROXY_PORT, "9053"));
            AvailableTor availableTor = new AvailableTor(torHiddenServiceDir.getName());
            networkNode = new TorNetworkNode(port,
                    new CoreNetworkProtoResolver(Clock.systemDefaultZone()),
                    false,
                    availableTor,
                    null);
            ...
            try {
                ...
                peerExchangeManager.requestReportedPeersFromSeedNodes(seedNodeRepository.getSeedNodeAddresses().iterator().next());
        ...
        if (lastRun != 0 && System.currentTimeMillis() - lastRun != 0) {
            ...
            keys.forEach(key -> {
                ...
                if (value != 0) {
                    String valueAsString = String.format("%.2f", value * perMinuteFactor);
                    report.put(key, valueAsString);
                }
            });
